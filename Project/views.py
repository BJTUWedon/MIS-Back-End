        ...
        try:
            ...
            if id == "-1":
                with open(src, 'wb')as f:
                    for ffile in file_obj.chunks():
                        f.write(ffile)
                File.objects.create(filename=title, type=type, content=content,createDate=datetime.datetime.now(), src=r"http://lvmaozi:9999/"+src)#我认为下面还要返回id
                ...
                with open(src, 'wb')as f:
                    for ffile in file_obj.chunks():
                        f.write(ffile)
                File.objects.filter(id=id).update(filename=title, type=type, content=content,createDate=datetime.datetime.now(),src=r"http://lvmaozi:9999/"+src)
                Data = {"title": title, "id": id}
        except Exception as e:
            success = False
            Data = str(e)
